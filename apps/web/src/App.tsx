...
function EpubReadalong({
  ...
  const [follow, setFollowState] = useState(() => window.localStorage.getItem("operalibre.readerFollow") !== "0");
  const setFollow = useCallback((value: boolean | ((prev: boolean) => boolean)) => {
    setFollowState((prev) => {
      const next = typeof value === "function" ? value(prev) : value;
      try {
        window.localStorage.setItem("operalibre.readerFollow", next ? "1" : "0");
      } catch {
        // ignore storage failures
      }
      return next;
  ...
  const [readerThemeChoice, setReaderThemeChoice] = useState<ReaderThemeChoice>(() =>
    readReaderThemeChoice(window.localStorage)
  );
  ...
  useEffect(() => {
    ...
    const handleRelocated = (nextLocation: Location) => {
      ...
      if (anchor && anchor !== anchorCfiRef.current) {
        ...
        try {
          window.localStorage.setItem(locationStorageKey, anchor);
        } catch {
          // ignore storage failures
        }
      }
    ...
    const openBook = async () => {
      try {
        ...
        try {
          savedLocation = window.localStorage.getItem(locationStorageKey);
        } catch {
          savedLocation = null;
        }
  ...
  useEffect(() => {
    writeReaderThemeChoice(window.localStorage, readerThemeChoice);
  }, [readerThemeChoice]);
