...
| C22 | The request timeout is scoped: uploads, book downloads, rescans, and Libation sign-in completion sit outside it, because each can legitimately run for minutes — a download builds its archive before the response begins, a rescan walks and fingerprints the whole library, and a sign-in completion waits on an external browser flow before its own scan. None of the four can hang without an operation behind it hanging first, but none is bounded either; a stuck scan or sign-in holds its connection until the client gives up. | Open | Phase 6 |
