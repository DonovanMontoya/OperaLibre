...
| C22 | The request timeout is scoped: uploads, book downloads, rescans, Libation sign-in completion, and the two update installers sit outside it, because each can legitimately run for minutes — a download builds its archive before the response begins, a rescan walks and fingerprints the whole library, a sign-in completion waits on an external browser flow before its own scan, and an installer is allowed ten minutes to fetch its release asset before extracting it. None of the six can hang without an operation behind it hanging first, but none is bounded either; a stuck scan, sign-in, or install holds its connection until the client gives up. The installers no longer leave their `installing` flag wedged if the future is dropped, so a cancelled install is recoverable without a restart. | Open | Phase 6 |
