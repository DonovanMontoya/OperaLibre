...
When a book has an EPUB companion, a *sync map* lets the reader pane follow the audio: the narrated sentence is highlighted, the page turns with the narration, and tapping any sentence plays from there. There are two levels of precision, and every EPUB gets at least the first:
