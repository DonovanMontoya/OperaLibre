...
When a book has an EPUB companion, a *sync map* lets the reader pane follow the audio: the narrated sentence is highlighted, the page turns with the narration, and tapping any sentence plays from there. These controls require the server’s follow-along experiment and the reader’s opt-in; chapter sync remains available when sentence following is off. There are two levels of map precision:
