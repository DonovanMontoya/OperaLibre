...
If you delete just one user, an authorized administrator can create them again with a new password. The server will not allow the final owner to be deleted or demoted. If you delete the file, restart the server and complete setup again. Setup follows the same per-mode rules as the first run, including the `proxy`-mode one-time token.
